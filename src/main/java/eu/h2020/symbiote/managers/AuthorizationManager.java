...
//	    		});
    ...
    private void enableSecurity() throws SecurityHandlerException {
        ...
        ///////////////////////////////////////////////////
        log.info("keystoreName = " + keystoreName);
        log.info("keystorePass = " + keystorePass);
        log.info("localAamAddress = " + localAamAddress);
        log.info("componentOwnerName = " + componentOwnerName);
        log.info("componentOwnerPassword = " + componentOwnerPassword);
        ////////////////////////////////////////////////
    ...
    private Set<String> checkStoredResourcePolicies(SecurityRequest request, String resourceId) {
        Set<String> ids = null;
        try {
            log.info("checkStoredResourcePolicies resourceId= " + resourceId);

            log.debug("Received a security request : " + request.toString());
            log.info("Received a security request : " + request.toString());
            ...
            if(accPolicy == null) {
                ...
                log.info("No access policies for resource " + resourceId);
                return Collections.emptySet();
            }
            
            log.info("checkStoredResourcePolicies 1");
            accessPolicyMap.put(resourceId, accPolicy.get().getPolicy());
            log.info("checkStoredResourcePolicies 2 accPolicy.get().getPolicy() = " + accPolicy.get().getPolicy());
            ...
            log.info("request.getSecurityCredentials().toArray()[0]: " +request.getSecurityCredentials().toArray()[0]);
            ids = componentSecurityHandler.getSatisfiedPoliciesIdentifiers(accessPolicyMap, request);//here
            log.info("checkStoredResourcePolicies 3");
        } catch (Exception e) {
            log.error("Exception thrown during checking policies: " + e.getMessage(), e);
        }
        log.info("checkStoredResourcePolicies returns ids = " + ids);
        return ids;
    }
